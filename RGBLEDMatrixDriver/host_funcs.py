...
def threadReadDataFromHostCOMForever(logger, port, data_lock, data):
    try:
        with Serial(**settings.HOST_COM_PORT_CONFIG) as ser:
            while(True):
                try:
                    ...
                    with data_lock:
                        data[:] = json.loads(data_bytes.decode('utf-8'))    # CAUTION: Make sure to use '[:]' to modify original list
